...
struct joystick_submodule gtk3_js_submod_keyboard = {
	.name = "keyboard",
...
#include "keyboard_gtk3_mappings.c"
...
static void emulator_command(struct ui_gtk3_interface *uigtk3, guint keyval, int shift) {
	(void)uigtk3;
...
gboolean gtk3_keyboard_handle_key_press(GtkWidget *widget, GdkEventKey *event, gpointer user_data) {
	...
	if (event->hardware_keycode >= MAX_KEYCODE) {
		...
		return gtk_window_activate_key(GTK_WINDOW(uigtk3->top_window), event);
	}
	...
	if (!control) {
		uigtk3->keyboard.control = 0;
	}

	for (unsigned i = 0; i < GTK_KBD_JS_MAX_AXES; i++) {
		if (uigtk3->keyboard.enabled_axis[i]) {
			if (keyval == uigtk3->keyboard.enabled_axis[i]->key0) {
				uigtk3->keyboard.enabled_axis[i]->value = 0;
				return FALSE;
			}
			if (keyval == uigtk3->keyboard.enabled_axis[i]->key1) {
				uigtk3->keyboard.enabled_axis[i]->value = 65535;
				return FALSE;
			}
		}
	}
	for (unsigned i = 0; i < GTK_KBD_JS_MAX_BUTTONS; i++) {
		if (uigtk3->keyboard.enabled_button[i]) {
			if (keyval == uigtk3->keyboard.enabled_button[i]->key) {
				uigtk3->keyboard.enabled_button[i]->value = 1;
				return FALSE;
			}
		}
	}
	...
	if (uigtk3->keyboard.control && gtk_window_activate_key(GTK_WINDOW(uigtk3->top_window), event) == TRUE) {
		return TRUE;
	}

	if (keyval == GDK_KEY_Control_L || keyval == GDK_KEY_Control_R) {
		uigtk3->keyboard.control = 1;
		return TRUE;
	}
	...
	if (uigtk3->keyboard.control) {
		emulator_command(uigtk3 ,keyval, shift);
		return TRUE;
	}
...
gboolean gtk3_keyboard_handle_key_release(GtkWidget *widget, GdkEventKey *event, gpointer user_data) {
	...
	for (unsigned i = 0; i < GTK_KBD_JS_MAX_AXES; i++) {
		if (uigtk3->keyboard.enabled_axis[i]) {
			if (keyval == uigtk3->keyboard.enabled_axis[i]->key0) {
				if (uigtk3->keyboard.enabled_axis[i]->value < 32768)
					uigtk3->keyboard.enabled_axis[i]->value = 32256;
				return FALSE;
			}
			if (keyval == uigtk3->keyboard.enabled_axis[i]->key1) {
				if (uigtk3->keyboard.enabled_axis[i]->value >= 32768)
					uigtk3->keyboard.enabled_axis[i]->value = 33280;
				return FALSE;
			}
		}
	}
	for (unsigned i = 0; i < GTK_KBD_JS_MAX_BUTTONS; i++) {
		if (uigtk3->keyboard.enabled_button[i]) {
			if (keyval == uigtk3->keyboard.enabled_button[i]->key) {
				uigtk3->keyboard.enabled_button[i]->value = 0;
				return FALSE;
			}
		}
	}
...
static struct joystick_axis *configure_axis(char *spec, unsigned jaxis) {
	...
	*axis = (struct gtk_kbd_js_axis){0};
	...
	for (unsigned i = 0; i < GTK_KBD_JS_MAX_AXES; i++) {
		if (!global_uigtk3->keyboard.enabled_axis[i]) {
			global_uigtk3->keyboard.enabled_axis[i] = axis;
			break;
		}
	}
...
static struct joystick_button *configure_button(char *spec, unsigned jbutton) {
	...
	*button = (struct gtk_kbd_js_button){0};
	...
	for (unsigned i = 0; i < GTK_KBD_JS_MAX_BUTTONS; i++) {
		if (!global_uigtk3->keyboard.enabled_button[i]) {
			global_uigtk3->keyboard.enabled_button[i] = button;
			break;
		}
	}
...
static void gtk_kbd_js_axis_free(void *sptr) {
	...
	for (unsigned i = 0; i < GTK_KBD_JS_MAX_AXES; i++) {
		if (axis == uigtk3->keyboard.enabled_axis[i]) {
			uigtk3->keyboard.enabled_axis[i] = NULL;
		}
	}
	free(axis);
}
...
static void gtk_kbd_js_button_free(void *sptr) {
	...
	for (unsigned i = 0; i < GTK_KBD_JS_MAX_BUTTONS; i++) {
		if (button == uigtk3->keyboard.enabled_button[i]) {
			uigtk3->keyboard.enabled_button[i] = NULL;
		}
	}
	free(button);
}
